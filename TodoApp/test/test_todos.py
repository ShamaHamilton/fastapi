...
def test_read_one_authenticated(test_todo):
    response: Response = client.get('/todos/todo/1')
    assert response.status_code == status.HTTP_200_OK
    assert response.json() == {
        'complete': False,
        'title': 'Learn to code!',
...
def test_create_todo(test_todo):
    ...
    response: Response = client.post('/todos/todo', json=request_data)
...
def test_update_todo(test_todo):
    ...
    response: Response = client.put('/todos/todo/1', json=request_data)
...
def test_update_todo_not_found(test_todo):
    ...
    response: Response = client.put('/todos/todo/999', json=request_data)
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {'detail': 'Todo not found.'}


def test_delete_todo(test_todo):
    response: Response = client.delete('/todos/todo/1')
    assert response.status_code == status.HTTP_204_NO_CONTENT
